#!/usr/bin/env python3
"""Create Thai stock scan report and optionally send to Telegram."""
...
def calc_persistence_map(all_stocks: pd.DataFrame) -> dict[str, str]:
    if all_stocks.empty or "symbol" not in all_stocks.columns:
        return {}
    required = {"date", "setup_score", "ret_10d_pct", "benchmark_ret_10d_pct"}
    ...
    for symbol, grp in df.groupby("symbol"):
        ...
        if len(grp) < 10:
            ...
            continue
        rs10 = pd.to_numeric(grp["ret_10d_pct"], errors="coerce") - pd.to_numeric(grp["benchmark_ret_10d_pct"], errors="coerce")
...
def build_report(stock_csv: str, passing_csv: str, sector_csv: str, top: int) -> str:
    ...
    lines = [
        f"หุ้นไทย วิเคราะห์หุ้น {data_date}",
    ...
    if passing.empty:
        ...
        for idx, row in passing.head(top).iterrows():
            ...
            rs10 = None
            r10 = to_float(row.get("ret_10d_pct"))
            b10 = to_float(row.get("benchmark_ret_10d_pct"))
            if r10 is not None and b10 is not None:
                rs10 = r10 - b10
            ...
            score_text = fmt_num(score) if score is not None else "n/a"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Thai stock scan report.")
    parser.add_argument("--stock-csv", default="thai_analyze_all.csv")
    parser.add_argument("--passing-csv", default="thai_analyze_passing.csv")
    parser.add_argument("--sector-csv", default="thai_sector_rotation.csv")
    parser.add_argument("--output", default="thai_summary.txt")
    parser.add_argument("--top", type=int, default=10)
    parser.add_argument("--telegram", action="store_true")
    parser.add_argument("--telegram-csv", action="append", default=[])
